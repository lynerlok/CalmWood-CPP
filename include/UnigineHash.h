...
protected:
	...
	UNIGINE_INLINE void realloc() { rehash(capacity == 0 ? 8 : capacity << 1); }

	UNIGINE_INLINE void rehash(Counter new_capacity)
	{
		...
		if (data != nullptr)
		{
			...
			for (Counter i = 0; i < capacity; ++i)
			{
				if (data[i] == nullptr)
					continue;
				Counter index = data[i]->hash & mask;
				while (new_data[index])
					index = (index + 1) & mask;

...
{
public:
	static bool convertible(ObjectIntersection *obj) { return obj && obj->getType() == ObjectIntersection::OBJECT_INTERSECTION_NORMAL; }
...
public:
	...
	void setBakeToVoxelProbe(bool enabled, int surface);
	bool getBakeToVoxelProbe(int surface) const;
...
public:
	...
	bool getSplineMesh(const Ptr<Mesh> &mesh) const;
	void flushMesh();
...
public:
	...
	bool getClutterTransforms(const UNIGINE_BOUND_BOX & bounds, Vector< Math::mat4 > &transforms, bool precise = true, bool force = true) const;
	bool getClutterWorldTransforms(const UNIGINE_BOUND_BOX & bounds, Vector< Math::Mat4 > &transforms, bool precise = true, bool force = true) const;
	int getClutterLocalTransforms(const BoundBox & bounds, Vector< Math::mat4 > &transforms, int precise = 1, int force = 1) const;
...
public:
	...
	bool isCollisionBicubicFilter() const;
	void setIntersectionBicubicFilterHeight(bool height);
	bool isIntersectionBicubicFilterHeight() const;
	void setIntersectionBicubicFilterNormal(bool normal);
	bool isIntersectionBicubicFilterNormal() const;
	void setIntersectionPrecisionBegin(float begin);
	float getIntersectionPrecisionBegin() const;
	void setIntersectionPrecisionEnd(float end);
	float getIntersectionPrecisionEnd() const;
...
public:
	...
	Math::vec3 getNormal() const;
	Math::vec3 getTangent() const;
	Math::vec3 getBinormal() const;
...
public:
	...
	static bool terrainLoad(const UNIGINE_BOUND_BOX & bb);
	...
	static void asyncTextureDraw(int operation_id, const UGUID & file_guid, const Math::ivec2 &coord, const Math::ivec2 &resolution, int flags_file_data, const Vector< UNIGINE_BOUND_BOX > &bounds_preload);
	...
	static void asyncTextureDraw(const UGUID & file_guid, const Math::ivec2 &coord, const Math::ivec2 &resolution, int flags_file_data, const Vector< UNIGINE_BOUND_BOX > &bounds_preload);
	...
	UNIGINE_DEPRECATED(static void asyncImageDraw(int operation_id, const UGUID & file_guid, const Math::ivec2 &coord, const Math::ivec2 &resolution, int flags_file_data));
	UNIGINE_DEPRECATED(static void asyncImageDraw(int operation_id, const UGUID & file_guid, const Math::ivec2 &coord, const Math::ivec2 &resolution));
	UNIGINE_DEPRECATED(static void asyncImageDraw(const UGUID & file_guid, const Math::ivec2 &coord, const Math::ivec2 &resolution, int flags_file_data));
	UNIGINE_DEPRECATED(static void asyncImageDraw(const UGUID & file_guid, const Math::ivec2 &coord, const Math::ivec2 &resolution));
	static void *addImageDrawCallback(Unigine::CallbackBase5< UGUID, int, Ptr<LandscapeImages>, Math::ivec2, int > *func);
	static bool removeImageDrawCallback(void *id);
	static void clearImageDrawCallbacks();
...
public:
	...
	UNIGINE_BOUND_BOX getWorldBoundBoxParticles() const;

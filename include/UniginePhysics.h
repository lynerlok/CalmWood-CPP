...
public:
	...
	static void setStable(bool stable); ///< "stable" mode means that objects are updated in a specific order
	static bool isStable();
	static void setFixed(bool fixed); ///< "fixed" mode caps render FPS to physics FPS (otherwise physics state can be interpolated between rendered frames)
	static bool isFixed();
	...
	static int saveState(const Ptr<Stream> &stream);
	static int restoreState(const Ptr<Stream> &stream);
	...
	static int isBody(int id);
	static Ptr<Body> getBody(int id);
	static int isJoint(int id);
	static Ptr<Joint> getJoint(int id);
	static int isShape(int id);

...
int Animal::detection ( Environment * environment )
{
    ...
    for( int cellX = 0; cellX < detectionRangeSize; ++cellX) {
        for ( int cellY = 0; cellY < detectionRangeSize; ++cellY) {
            VisibleAnimals.push_back(environment->getCell ( detectionRange[cellX][cellY], detectionRange[cellX][cellY] )->getCellContentAnimals());
            VisiblePlants.push_back(environment->getCell ( detectionRange[cellX][cellY], detectionRange[cellX][cellY] )->getCellContentPlants());
            CellSpecs.push_back(environment->getCell ( detectionRange[cellX][cellY], detectionRange[cellX][cellY] )->getCellContentSpecs());
        }
    }
